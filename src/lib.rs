...
// TODO: add architecture dependent register recognizing?
...
pub mod ir {
    ...
    pub mod index {
        ...
        impl Register {
            pub fn from<A: super::Architecture>(name: &str) -> Option<Self> {
        ...
        fn from_ast<'src>(
            ...
            match rvalue {
                crate::ast::Rvalue::Label(label) => {
                    Self::Label(labels.get_or_add_label_index(label))
                }
                ...
                Some(stmt) => match stmt {
                    ...
                    crate::ast::Statement::Return(expr) => {
                        match expr_as_br_cond(expr, &binding_map, label_map) {
                            ...
                            Err(other) => {
                                End::TailValue(expr_as_value(other, &binding_map, label_map))
                            }
                        }
                    }
                },
